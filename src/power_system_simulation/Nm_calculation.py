...
"""
def Nm_function(given_lineID: int, input_data_path: str, metadata_path: str, active_power_profile_path: str, reactive_power_profile_path: str) -> list[int]:
    ...
    if input_data['line']['from_status'][input_data['line']['id'] == given_lineID] == 0 or input_data['line']['to_status'][input_data['line']['id'] == given_lineID] == 0:
    ...
    for x in alt_list:
        ...
        with Timer("Batch Calculation using the linear method"):
            output_data = model.calculate_power_flow(update_data=update_data, calculation_method=CalculationMethod.linear)
        ######### OUTPUT TABLE
        ...
        timestamp_index = np.where(output_data['line']['loading'] == max_line_load )[0][0] #this returns a number from 0 to 959
